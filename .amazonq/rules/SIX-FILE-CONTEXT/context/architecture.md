...
| Auth      | Supabase Auth               | Trader authentication, secure JWT generation, and session management |
...
- `src/engine` — Owns the core business logic. **Must utilize native PostgreSQL graph traversal (Recursive CTEs via Prisma `$queryRaw`) for fraud detection to prevent microservice/Python overhead.**
...
- **Trader (Core Identity & State)**:
  ...
  - **Key Fields**: `id`, `phoneNumber`, `businessName`, `squadVirtualAccount`, `currentScore` (default: 0), `activeTier`.
  - **Why**: The system needs a fast way to query a user's current score for the Lender API without recalculating their entire history on every request.
- **Transaction (The Event Stream)**:
  ...
  - **Why**: The AI scoring engine and fraud detection modules require raw historical data to calculate consistency, detect round-number clustering, and identify circular fraud.
- **ScoreLedger (Historical Trust Tracking)**:
  - **Purpose**: An append-only log tracking every time a trader's score changes.
  - **Key Fields**: `id`, `traderId`, `scoreChange` (+15, -50), `newTotalScore`, `reason` (e.g., "On-time Ajo payment", "Velocity anomaly detected"), `timestamp`.
...
- Traders authenticate to the internal dashboard using Supabase Auth, which issues JWTs for secure session management.
...
3. **Data Source Authority**: The AI Scoring Engine must never use self-reported transaction data; all financial data feeding the score must originate exclusively from verified Squad API webhooks.
